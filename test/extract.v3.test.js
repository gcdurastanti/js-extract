describe('extract v3 chainable & custom API', () => {
  const data = { 1: { val: 1 }, 2: { val: 2 } };

  it('should support .map() to transform extracted values', () => {
    const extract = require('../src/extract.v3').default;
  ...
  it('should support .filter() to filter extracted values', () => {
    ...
    const result = extract({ 1: ['val'], 2: ['val'] })
      .filter(x => x > 1)
      .from(data);
    assert.deepEqual(result, { 2: { val: 2 } });
  });
});
describe('extract v3 performance', () => {
  const largeData = {};
  ...
  it('should extract from large datasets quickly', () => {
    ...
    const result = require('../src/extract.v3').default(query).from(largeData);
...
import extract from '../src/extract.v3';
